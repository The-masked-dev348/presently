...
describe("Presently product rules", () => {
  ...
  it("only treats a file id as owned when its recorded owner matches the requester", () => {
    ...
    expect(isOwnedFileId(42, 9, 7)).toBe(false); // file 42 belongs to user 9, requested by user 7
    expect(isOwnedFileId(42, undefined, 7)).toBe(false); // file id doesn't exist at all
    expect(isOwnedFileId(null, undefined, 7)).toBe(true); // clearing the field — nothing to check
    expect(isOwnedFileId(undefined, undefined, 7)).toBe(true); // field left untouched — nothing to check
  });
