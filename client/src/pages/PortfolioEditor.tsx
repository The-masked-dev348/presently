...
const emptyProject = { title: "", description: "", clientProblem: "", solution: "", businessImpact: "", technologies: "", liveUrl: "", githubUrl: "" };
...
export default function PortfolioEditor() {
  ...
  const preview = useMemo<PreviewPortfolio>(() => ({
    ...
    projects: (projectsQuery.data ?? []).map(item => {
      ...
      return { id: item.id, title: item.title, description: item.description, clientProblem: item.clientProblem, solution: item.solution, businessImpact: item.businessImpact, technologies: item.technologies, liveUrl: item.liveUrl, githubUrl: item.githubUrl, media: [...localMedia, ...(item.media ?? []).filter(media => !localIds.has(media.id))] };
    }),
  ...
  const resetProject = () => { setProject(emptyProject); setEditingProjectId(null); setPendingProjectMediaFiles([]); };
  ...
  const saveProject = async () => {
    ...
    const payload = { title: project.title, description: project.description, clientProblem: project.clientProblem, solution: project.solution, businessImpact: project.businessImpact, technologies: project.technologies.split(",").map(item => item.trim()).filter(Boolean), liveUrl: project.liveUrl, githubUrl: project.githubUrl };
    try {
  ...
  const startEdit = (item: NonNullable<typeof projectsQuery.data>[number]) => { setEditingProjectId(item.id); setProject({ title: item.title, description: item.description || "", clientProblem: item.clientProblem || "", solution: item.solution || "", businessImpact: item.businessImpact || "", technologies: item.technologies || "", liveUrl: item.liveUrl || "", githubUrl: item.githubUrl || "" }); setPendingProjectMediaFiles([]); setActiveTab("work"); };
