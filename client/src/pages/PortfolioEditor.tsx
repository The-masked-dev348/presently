...
export default function PortfolioEditor() {
  ...
  const saveProject = async () => {
    ...
    const payload = { title: project.title, description: project.description, technologies: project.technologies.split(",").map(item => item.trim()).filter(Boolean), liveUrl: project.liveUrl, githubUrl: project.githubUrl };
    try {
  ...
  const startEdit = (item: NonNullable<typeof projectsQuery.data>[number]) => { setEditingProjectId(item.id); setProject({ title: item.title, description: item.description || "", technologies: item.technologies || "", liveUrl: item.liveUrl || "", githubUrl: item.githubUrl || "" }); setPendingProjectMediaFiles([]); setActiveTab("work"); };
